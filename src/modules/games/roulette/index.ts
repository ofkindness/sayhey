...
const sendDelayedMessage =
  (bot: any) =>
  (chatId: string, text: string, delay = 0) =>
    new Promise((resolve) => {
      setTimeout(() => {
...
const sendDelayedSticker =
  (bot: any) =>
  (chatId: string, sticker: string, delay = 0) =>
    new Promise((resolve) => {
      setTimeout(() => {
...
function* generateRound(roundCount: number, chatId: string, player: any, bot: any) {
...
function* rouletteRound(chatId: string, player: any, bot: any) {
  const init = format(i18next.t('init'), getName(player).indexOf(' ') > 0 ? getName(player) : `@${getName(player)}`);
...
const exec = (generator: any, yieldValue: string) => {
  const next = generator.next(yieldValue);
  if (!next.done) {
    return next.value.then(
      (result: any) => exec(generator, result),
      (err: any) => generator.throw(err),
    );
  }
...
dispatcher.command('/roulette', async (req: any, res: any) => {
  const {
    chat: { id: chatId },
    from: player,
  } = req;
